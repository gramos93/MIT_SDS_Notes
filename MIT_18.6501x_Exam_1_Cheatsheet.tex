...
where $\mathbb{B}(t)$ is a Brownian bridge distribution (Donker's Theorem), which is a pivotal distribution and it's quantiles can be obtained in tables. 
The Ks test with asymptotic level $\alpha$
...
$\tilde{\pi}(.) \text{ of } \eta \text{ satisfies } \pi(\eta) \propto \sqrt{\tilde{I}(\eta)}$. We can write $\tilde{\pi}(\eta)$ in term of 
$\phi^{'}(.) \text{ and } \phi^{-1}(.)$:
...
\begin{itemize}[itemsep=0em]
    \item Random component: $Y|X = x \sim (Ber, Exp, Poisson, etc)$
    \item Regression Function: $g(\mu(x)) = x^T \beta$, where $g$ is called the \emph{link function}
          and $\mu(x)$ is the regression function.
\end{itemize}
...
$$f_\theta(y) = exp\left[\sum^{h}_{i=1} \eta_i(\theta) T_i(y) - B(\theta) \right] h(y)$$

A distribution is said to be a \emph{k-parameter} exponential on $\mathbb{R}^q$ if there exist real valued functions $\eta_1, \eta_2, \dots$ and  $B$ of $\theta$,
$T_1, T_2, ..., T_k$ and $h$ of $y \in \mathbb{R}^q$ such that the PDF can be wrtitten as above.

\begin{tabular}{l c}
    \textit{Distribution} &\textit{Exponential Form}\\
    Bernoulli & $exp\left[y log(\theta / (1 - \theta)) - (-log(1 - \theta)) \right]$\\
    Poisson & $(1/y!) \: exp\left[-\theta + y log(\theta) \right]$\\
...
A exponential distribution can be called canonical if it has only one parameter, $k = 1$ and the \emph{dispersion parameter} $\phi$ is known (see bellow).
In the case that $\phi$ is unknown, the distribution may or may not be canonical. This is the case of the Normal distribution, if $\sigma^2 \text{ or }\mu$ are 
known, than we only have on parameter and the distribution is canocical, if both are unknown, the distribution is not.
The canocical form is :
...
$$\ell_n (\mathbb{Y}, \mathbb{X}, \mathcal{\beta}) = \sum^{n}_{i=1} \frac{Y_i \theta_i - b(\theta_i)}{\phi} + const$$

When the \emph{canocical link} is used, the log-likelihood has a simpler form. Furthermore, is $\phi > 0$ and $rank(\mathbb{X}) = p$, 
the log-likelihood is \emph{strictly concave}. As a concequence, \emph{the MLE is unique}. However, if another parametrization is used,
the log-likelihood may not be strictly concave leading to \emph{several local maxima}. In this case optimization algorithms may be used.

$$\ell_n (\mathbb{Y}, \mathbb{X}, \mathcal{\beta}) = \sum^{n}_{i=1} \frac{Y_i X_i \beta - b(X_i \beta)}{\phi} + const$$

\subsubsection{Models}

Let $(X_i, Y_i) \in \mathbb{R}^p \times \mathbb{R}$, be independent random pairs such that the conditioal distribution of $Y_i|X_I = x_i$
has density in the canocical exp. family. Here the means $\mu_i$ are related to the canocical parameter $\theta_i$ via $\mu_i = b'(\theta)$
and $\mu_i$ depends linearly on the covariates through a link function $g(\mu_i) = X_i^T \beta$.\\
\emph{Back to} $\beta$: given the link function $g$, note the following relationship between $\beta$ and $\theta$.

$$\theta_i = (b')^{-1}(\mu_i) = (b')^{-1}(g^{-1}(X_I^T \beta)) \equiv h(X_I^T \beta)$$

\emph{Note:} remark that if $g$ is the canonical link function, $h$ is the identity, which means that $\theta_i = X_i^T \beta$.
